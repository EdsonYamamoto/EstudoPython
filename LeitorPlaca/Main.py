...
import LeitorPlaca.DetectarPlacas
...
def main():
    ...
    while (True):
        ...
        listaDePossiveisPlacas = LeitorPlaca.DetectarPlacas.DetectarPlacasInScene(imgCenaOriginal)
        # detectar Placas

        listaDePossiveisPlacas = LeitorPlaca.DetectarCaracteres.DetectarCaracteresNasPlacas(listaDePossiveisPlacas)
        ...
        if len(listaDePossiveisPlacas) != 0:
            ...
            if len(licPlaca.strCaracteres) != 0:
                ...
                stringData = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                ...
                if placaCarro.match(licPlaca.strCaracteres):
                    ...
                    cv2.imwrite("imgOriginal"+stringData+".png", imgCenaOriginal)
                    cv2.imwrite("imgPlaca"+stringData+".png", licPlaca.imgPlaca)
                    ...
                    cv2.imwrite("imgTentativaOriginal"+stringData+".png", imgCenaOriginal)
                    cv2.imwrite(stringData+"-imgTentativaPlaca.png", licPlaca.imgPlaca)
                    cv2.imwrite(stringData+"-imgTentativaThreshold.png", licPlaca.imgThreshold)
