...
from LeitorPlaca.DetectarCaracteres import DetectarCaracteres
from LeitorPlaca.DetectarPlacas import DetectarPlacas


class LeitorPlacaClass:

    ###################################################################################################
    def MetodoInicial(self):

        print("estou imprimindo")
        blnKNNTrainingSuccessful = DetectarCaracteres.loadKNNDataAndTrainKNN(object)
        ...
        imgCenaOriginal = cv2.imread("7.png")
        # abrir imagem
        ...
        listaDePossiveisPlacas = DetectarPlacas.DetectarPlacasInScene(imgCenaOriginal, object)
        # detectar Placas
        ...
        # exibir imagem que foi escolhida cv2.imread("imagens/7.png")
        ...
        if len(listaDePossiveisPlacas) == 0:
            ...
            LeitorPlacaClass.desenharRetanguloVermelhoAoRedorDaPlaca(imgCenaOriginal, licPlaca)
            ...
            LeitorPlacaClass.escreverCaracteresDaPlacaNaImagem(imgCenaOriginal, licPlaca)
    ...
    def desenharRetanguloVermelhoAoRedorDaPlaca(imgCenaOriginal, licPlaca):
        ...
        cv2.line(imgCenaOriginal, tuple(p2fRectPoints[0]), tuple(p2fRectPoints[1]), LeitorPlacaClass.ESCALA_VERMELHO, 2)
        # desenhar 4 linhas vermelhas
        cv2.line(imgCenaOriginal, tuple(p2fRectPoints[1]), tuple(p2fRectPoints[2]), LeitorPlacaClass.ESCALA_VERMELHO, 2)
        cv2.line(imgCenaOriginal, tuple(p2fRectPoints[2]), tuple(p2fRectPoints[3]), LeitorPlacaClass.ESCALA_VERMELHO, 2)
        cv2.line(imgCenaOriginal, tuple(p2fRectPoints[3]), tuple(p2fRectPoints[0]), LeitorPlacaClass.ESCALA_VERMELHO, 2)
    ...
    def escreverCaracteresDaPlacaNaImagem(imgCenaOriginal, licPlaca):
        ...
        cv2.putText(imgCenaOriginal, licPlaca.strCaracteres, (ptLowerLeftTextOriginX, ptLowerLeftTextOriginY), intFontFace,
                    fltFontScale, LeitorPlacaClass.ESCALA_AMARELO, intFontThickness)
