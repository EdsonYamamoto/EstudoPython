...
import LeitorPlaca.Preprocesso as Preprocesso
import LeitorPlaca.DetectarCaracteres as DetectarCaracteres
import LeitorPlaca.PossivelPlaca as PossivelPlaca
import LeitorPlaca.PossivelCaractere as PossivelCaractere

class DetectarPlacas:
    ...
    ###################################################################################################
    def DetectarPlacasInScene(imgCenaOriginal, self):
        ...
        imgEscalaDeCinzaScene, imgThresholdScene = Preprocesso(imgCenaOriginal)
        ...
        listaDePossiveisCaracteresInScene = DetectarPlacas.findPossivelCaracteresInScene(imgThresholdScene)
        ...
        for listaDeCombinacaoDeCaracteres in listaDeListasDeCombinacaoDeCaracteresInScene:       # Para cada grupo de gráficos correspondentes
            possivelPlaca = DetectarPlacas.extrairPlaca(imgCenaOriginal, listaDeCombinacaoDeCaracteres)         # tentar extrair Placa
        ...
        if LeitorPlacaClass.mostrarPassos == True: # Mostrar etapas #######################################################
            ...
            for i in range(0, len(listaDePossiveisPlacas)):
                ...
                cv2.line(imgContours, tuple(p2fRectPoints[0]), tuple(p2fRectPoints[1]), LeitorPlacaClass.ESCALA_VERMELHO, 2)
                cv2.line(imgContours, tuple(p2fRectPoints[1]), tuple(p2fRectPoints[2]), LeitorPlacaClass.ESCALA_VERMELHO, 2)
                cv2.line(imgContours, tuple(p2fRectPoints[2]), tuple(p2fRectPoints[3]), LeitorPlacaClass.ESCALA_VERMELHO, 2)
                cv2.line(imgContours, tuple(p2fRectPoints[3]), tuple(p2fRectPoints[0]), LeitorPlacaClass.ESCALA_VERMELHO, 2)
    ...
    def findPossivelCaracteresInScene(imgThreshold):
        ...
        for i in range(0, len(contornos)):                       # para cada contorno
            ...
            if LeitorPlacaClass.mostrarPassos == True: # Mostrar etapas ###################################################
                cv2.drawContours(imgContours, contornos, i, LeitorPlacaClass.ESCALA_BRANCO)
            # end if # Mostrar etapas #####################################################################
    ...
    def extrairPlaca(imgOriginal, listaDeCombinacaoDeCaracteres):
        ...
        intPlacaWidth = int((listaDeCombinacaoDeCaracteres[len(listaDeCombinacaoDeCaracteres) - 1].intBoundingRectX + listaDeCombinacaoDeCaracteres[len(listaDeCombinacaoDeCaracteres) - 1].intBoundingRectWidth - listaDeCombinacaoDeCaracteres[0].intBoundingRectX) * DetectarPlacas.PLACA_LARGURA_FATOR_PREENCHIMENTO)
        ...
        intPlacaHeight = int(fltAverageCaractereHeight * DetectarPlacas.PLACA_ALTURA_FATOR_PREENCHIMENTO)
