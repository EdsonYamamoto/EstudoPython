...
import LeitorPlaca.Main
import LeitorPlaca.Preprocesso
import LeitorPlaca.DetectarCaracteres
import LeitorPlaca.PossivelPlaca
import LeitorPlaca.PossivelCaractere
...
def DetectarPlacasInScene(imgCenaOriginal):
    ...
    if LeitorPlaca.Main.mostrarPassos == True: # Mostrar etapas #######################################################
        cv2.imshow("0", imgCenaOriginal)
    # end if # Mostrar etapas #########################################################################

    imgEscalaDeCinzaScene, imgThresholdScene = LeitorPlaca.Preprocesso.Preprocesso(imgCenaOriginal)
    ...
    if LeitorPlaca.Main.mostrarPassos == True: # Mostrar etapas #######################################################
        ...
        cv2.drawContours(imgContours, contornos, -1, LeitorPlaca.Main.ESCALA_BRANCO)
    ...
    listaDeListasDeCombinacaoDeCaracteresInScene = LeitorPlaca.DetectarCaracteres.findListOfListsOfMatchingCaracteres(listaDePossiveisCaracteresInScene)
    ...
    if LeitorPlaca.Main.mostrarPassos == True: # Mostrar etapas #######################################################
        ...
        for i in range(0, len(listaDePossiveisPlacas)):
            ...
            cv2.line(imgContours, tuple(p2fRectPoints[0]), tuple(p2fRectPoints[1]), LeitorPlaca.Main.ESCALA_VERMELHO, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[1]), tuple(p2fRectPoints[2]), LeitorPlaca.Main.ESCALA_VERMELHO, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[2]), tuple(p2fRectPoints[3]), LeitorPlaca.Main.ESCALA_VERMELHO, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[3]), tuple(p2fRectPoints[0]), LeitorPlaca.Main.ESCALA_VERMELHO, 2)
...
def findPossivelCaracteresInScene(imgThreshold):
    ...
    for i in range(0, len(contornos)):                       # para cada contorno
        ...
        if LeitorPlaca.Main.mostrarPassos == True: # Mostrar etapas ###################################################
            cv2.drawContours(imgContours, contornos, i, LeitorPlaca.Main.ESCALA_BRANCO)
        # end if # Mostrar etapas #####################################################################

        possivelCaractere = LeitorPlaca.PossivelCaractere.PossivelCaractere(contornos[i])

        if LeitorPlaca.DetectarCaracteres.verificaSePossivelCaractere(possivelCaractere):         # Se o contorno é uma possível char, note que este não se compara a outros caracteres (até o momento) . . .
...
def extrairPlaca(imgOriginal, listaDeCombinacaoDeCaracteres):
    possivelPlaca = LeitorPlaca.PossivelPlaca.PossivelPlaca()           # este será o valor de retorno
    ...
    fltHypotenuse = LeitorPlaca.DetectarCaracteres.distanciaEntreCaracteres(listaDeCombinacaoDeCaracteres[0], listaDeCombinacaoDeCaracteres[len(listaDeCombinacaoDeCaracteres) - 1])
