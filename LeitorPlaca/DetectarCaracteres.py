...
from LeitorPlaca.Preprocesso import Preprocesso
from LeitorPlaca.PossivelCaractere import PossivelCaractere

class DetectarCaracteres:
    ...
    def loadKNNDataAndTrainKNN(self):
        ...
        DetectarCaracteres.kNearest.setDefaultK(1)
        # conjunto padrão K para 1

        DetectarCaracteres.kNearest.train(npaFlattenedImages, cv2.ml.ROW_SAMPLE, npaClassifications)
    ...
    def DetectarCaracteresNasPlacas(listaDePossiveisPlacas):
        ...
        for possivelPlaca in listaDePossiveisPlacas:
            ...
            if LeitorPlacaClass.mostrarPassos == True:  # mostrar etapas ###################################################
            ...
            listaDePossiveisCaracteresInPlaca = DetectarCaracteres.encontrarPossivelCaractereNaPlaca(possivelPlaca.imgEscalaDeCinza,
            ...
            if LeitorPlacaClass.mostrarPassos == True:  # mostrar etapas  ###################################################
                ...
                cv2.drawContours(imgContours, contornos, -1, LeitorPlacaClass.ESCALA_BRANCO)

                cv2.imshow("6", imgContours)
                # end if # mostrar etapas #####################################################################

                # dada uma lista de todos os caracteres possíveis, encontrar grupos de caracteres correspondentes dentro da Placa
            listaDeListasDeCombinacaoDeCaracteresInPlaca = DetectarCaracteres.findListOfListsOfMatchingCaracteres(
            ...
            for i in range(0, len(
                ...
                listaDeListasDeCombinacaoDeCaracteresInPlaca[i] = DetectarCaracteres.removerSobreposicaoDeCaracteres(
            ...
            possivelPlaca.strCaracteres = DetectarCaracteres.recognizeCaracteresInPlaca(possivelPlaca.imgThreshold,
        ...
        if LeitorPlacaClass.mostrarPassos == True:
            print (
    ...
    def encontrarPossivelCaractereNaPlaca(imgEscalaDeCinza, imgThreshold):
        ...
        for contour in contornos:  # para cada contorno
            ...
            if DetectarCaracteres.verificaSePossivelCaractere(
    ...
    def verificaSePossivelCaractere(possivelCaractere):
        ...
        if (possivelCaractere.intBoundingRectArea > DetectarCaracteres.MIN_PIXEL_AREA and
                    possivelCaractere.intBoundingRectWidth > DetectarCaracteres.MIN_PIXEL_LARGURA and possivelCaractere.intBoundingRectHeight > DetectarCaracteres.MIN_PIXEL_ALTURA and
                DetectarCaracteres.MIN_ASPECT_RATIO < possivelCaractere.fltAspectRatio and possivelCaractere.fltAspectRatio < DetectarCaracteres.MAX_ASPECT_RATIO):
            return True
        else:
            return False
            # end if
    ...
    def findListOfListsOfMatchingCaracteres(listaDePossiveisCaracteres):
        ...
        for possivelCaractere in listaDePossiveisCaracteres:
            ...
            listaDeCombinacaoDeCaracteres = DetectarCaracteres.encontrarListaDeCombincacaoDeCaracteres(possivelCaractere,
            ...
            if len(listaDeCombinacaoDeCaracteres) < DetectarCaracteres.MIN_NUMERO_DE_COMBINACAO_CARACTERES:
            ...
            recursiveListOfListsOfMatchingCaracteres = DetectarCaracteres.findListOfListsOfMatchingCaracteres(
                listaDePossiveisCaracteresComAtualCombinacaoRemovida)
            # recursive call
    ...
    def encontrarListaDeCombincacaoDeCaracteres(possivelCaractere, listOfCaracteres):
        ...
        for possivelCombinacaoDeCaractere in listOfCaracteres:
            ...
            fltDistanciaEntreCaracteres = DetectarCaracteres.distanciaEntreCaracteres(possivelCaractere, possivelCombinacaoDeCaractere)

            fltAnguloEntreCaracteres = DetectarCaracteres.anguloEntreCaracteres(possivelCaractere, possivelCombinacaoDeCaractere)
            ...
            if (fltDistanciaEntreCaracteres < (possivelCaractere.fltDiagonalSize * DetectarCaracteres.MAX_DIAG_SIZE_MULTIPLE_AWAY) and
                        fltAnguloEntreCaracteres < DetectarCaracteres.MAX_ANGULO_ENTRE_CARACTERES and
                        fltChangeInArea < DetectarCaracteres.MAX_MUDANCA_EM_AREA and
                        fltChangeInWidth < DetectarCaracteres.MAX_MUDANCA_NA_LARGURA and
                        fltChangeInHeight < DetectarCaracteres.MAX_MUDANCA_NA_ALTURA):
        ...
        for caractereAtual in listaDeCombinacaoDeCaracteres:
            for outroCaractere in listaDeCombinacaoDeCaracteres:
                if caractereAtual != outroCaractere:
                    ...
                    if DetectarCaracteres.distanciaEntreCaracteres(caractereAtual, outroCaractere) < (
                        caractereAtual.fltDiagonalSize * DetectarCaracteres.MIN_DIAG_SIZE_MULTIPLE_AWAY):
    ...
    def recognizeCaracteresInPlaca(imgThreshold, listaDeCombinacaoDeCaracteres):
        ...
        for caractereAtual in listaDeCombinacaoDeCaracteres:
            ...
            cv2.rectangle(imgThresholdColor, pt1, pt2, LeitorPlacaClass.ESCALA_VERDE, 2)
            ...
            imgROIResized = cv2.resize(imgROI, (DetectarCaracteres.REDIMENSIONAR_CHAR_IMAGEM_LARGURA,
                                                DetectarCaracteres.REDIMENSIONAR_CHAR_IMAGEM_ALTURA))  # resize image, this is necessary for char recognition

            npaROIResized = imgROIResized.reshape((1,
                                                   DetectarCaracteres.REDIMENSIONAR_CHAR_IMAGEM_LARGURA * DetectarCaracteres.REDIMENSIONAR_CHAR_IMAGEM_ALTURA))  # flatten image into 1d numpy array

            npaROIResized = np.float32(npaROIResized)
            # convert from 1d numpy array of ints to 1d numpy array of floats

            retval, npaResults, neigh_resp, dists = DetectarCaracteres.kNearest.findNearest(npaROIResized,
        ...
        if LeitorPlacaClass.mostrarPassos == True:  # show steps #######################################################
